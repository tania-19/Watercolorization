...
def SaliencyDetection(image):
    ...
    element = cv2.getStructuringElement(cv2.MORPH_RECT, (7, 7),(-1, -1))
    cv2.dilate(saliencyBinaryMap,element, 4)
...
def SaliencyDistanceField(image, graph=True):
    #opcion 1
    mask=SaliencyDetection(image)
